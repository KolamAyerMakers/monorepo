...
Once S8 is released, `guide now` starts troubleshooting from a working course service, without requiring earlier exercise completions. Challenges affect the learner's service or generated website; further calls inspect the same challenge. Ask `guide` for hints; completion requires a working site and a one-sentence explanation of the cause and repair. An accepted repair receives a short congratulations and starts the next guided challenge in the same reply, without another `guide now`. Completion is committed before the next launch; launch failures preserve that credit and report the problem. The CLI and daemon must be deployed together: an answer permits one healthy inspection followed by at most one new launch. Source files and other accounts are not modified.
...
Daemon-side deterministic validation runs as the unprivileged bot user. The S6 behavioral suite and S8 service-lab actions execute only in the invoking learner's CLI under that learner's UID, never in the daemon. Operators must not run validation as root, add privileged wrappers, or bypass Unix permissions to make a quest pass.
