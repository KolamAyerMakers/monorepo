...
Once S8 is released, `guide now` starts troubleshooting from a working course service, without requiring earlier exercise completions. Challenges affect the learner's service or generated website; further calls inspect the same challenge. Ask `guide` for hints; completion requires a working site and a one-sentence explanation of the cause and repair. Source files and other accounts are not modified.
