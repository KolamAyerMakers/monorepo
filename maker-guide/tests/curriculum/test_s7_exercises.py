...
"""
                + next(
                    ...
                    for line in (
                        ...
                        .joinpath(f"content/lf2607/sessions/S07/{document_name}")
                        .read_text(encoding="utf-8")
                        .splitlines()
                    )
                    if line.startswith("printf 'GET ")
                ),
            ],
            cwd=temporary_path,
            env={"PATH": str(temporary_path), "USER": "s7-learner", "CAT": cat_path},
            capture_output=True,
            check=False,
            timeout=5,
        )
        assert completed_process.returncode == 0, completed_process.stderr
        assert completed_process.stderr == b"-w\n3\n127.0.0.1\n80\n", document_name
        assert completed_process.stdout == (
            b"GET /~s7-learner/ HTTP/1.1\r\n"
            b"Host: lf2607.kolamayermakers.org\r\n"
            b"Connection: close\r\n\r\n"
        ), document_name
