...
Personal Caddy listens for HTTP on all interfaces; the classroom firewall blocks new direct external connections to its port. Shared Caddy handles public HTTPS and forwards requests over loopback. Same software, two separate processes. Do not add `--domain` or change shared configuration. Use `systemctl --user` to control your service, never `caddy stop` or `caddy reload`, which can target shared Caddy. The explicit root is not a sandbox; keep secrets and symlinks to private files out of the published tree.
