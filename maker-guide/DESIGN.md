...
The checker interface is `site-check.sh PAGE [PAGE ...]`: accept arbitrary page paths, with an empty argument selecting the homepage. Zero arguments must print usage and exit nonzero without requesting a page; the reference exits `2`. The Linux-only suite parses the same private, read-only, unlinked temporary source snapshot with `bash -n` via `/proc/self/fd`, then runs it with case-specific arguments:

| Case ID | Arguments | Simulated requests | Required behavior |
|---|---|---|---|
| `both-ok` | `"" maker-report.html` | Both pages return HTTP `200` | Identify both pages and their successful `200` results. |
| `report-missing` | `maker-report.html` | Report `404` | Diagnose report `404`; name `maker-report.sh`, then `build-website` as repair advice. |
| `homepage-missing` | `""` | Homepage `404` | Diagnose homepage `404` without recommending report regeneration. |
| `http-error` | `maker-report.html ""` | Report `500`, homepage `200` | Diagnose report `500` as an error, not success. |
| `homepage-connection-failed` | `"" maker-report.html` | Homepage transport failure, report `200` | Diagnose the failure and still check the report. |
| `report-connection-failed` | `maker-report.html ""` | Report transport failure, homepage `200` | Diagnose the failure and still check the homepage. |
| `misleading-status` | `maker-report.html ""` | Both emit `200`, but report curl exits nonzero | Treat the failed curl request as failure despite the emitted `200`. |
| `missing-page` | `not-a-page.html` | Unknown page `404` | Identify that page and `404` without recommending report regeneration. |
| `usage` | None | No requests | Print usage help and exit nonzero. |

Only the selected fixture endpoints must receive requests in each passing case; the usage case must make none. Output identifies each page by its learner URL or a homepage/home page/report label, and unknown pages by their path or URL, on the diagnosis line or a preceding heading. Matching is case-insensitive and independent of page order. A healthy page needs `200` without failure wording. An HTTP failure needs the actual code and a diagnosis such as `missing`, `check`, `error`, or `unexpected`; a transport failure needs error, failure, connection, DNS, TLS, or timeout wording. Failures must not also claim success. Repair advice can follow on separate lines but must include the command names; report regeneration is only appropriate for `maker-report.html`. These are deterministic output rules, not arbitrary prose interpretation or LLM grading. A nonzero script exit status is not itself a grading failure; the reference script does not provide an aggregate health verdict through its exit status. The usage case specifically requires a nonzero exit.

#### Execution And Trust

The daemon never executes learner scripts. It reads source only for the bounded artifact and digest checks through normal Unix permissions. Execution belongs exclusively to `maker_guide.cli.site_check` in the learner's CLI, with root, mismatched real/effective UIDs, and daemon accounts rejected. The source must be a regular UTF-8 text file, at most 1 MiB, within the learner's home after path resolution. The runner uses a temporary working directory, case-specific page arguments, no interactive input, and a sanitized environment containing account-derived `HOME`, `USER`, `LOGNAME`, a fixed PATH with the shim first, and the C locale. Each Bash subprocess has a three-second limit and a 64 KiB combined stdout/stderr limit; the whole suite has a 25-second budget. Cleanup kills the child process group.

The PATH-based curl shim redirects allowed learner page URLs on `lf2607.kolamayermakers.org` or `lf-dev.kolamayermakers.org` to local HTTP fixtures, including the unknown-page `404` fixture. It delegates option handling to native `/usr/bin/curl`, rather than recognizing a reference command's flag order. This is URL substitution for simulated testing, not a sandbox: learner scripts retain their UID's permissions and can write files, launch processes, or bypass the shim. Sanitized environment, process cleanup, and time/output bounds do not promise isolation or absence of side effects. Never execute this runner as the bot, through sudo, or remotely on a learner's behalf.

The daemon authenticates the local caller with `SO_PEERCRED` and accepts at most one bounded continuation report on the same active connection. The strict report schema contains a version, source SHA-256, the complete fixed case IDs above with boolean results, and an optional fixed error token. Duplicate keys, unknown fields/cases, malformed values, and a mismatched digest are rejected. The daemon retains the selected course, task type/ID, session ID, and assignment/objective evidence window, then reselects the task and rehashes the source before recording completion. The CLI also checks the digest before execution and compares the file afterward. A changed source requires a fresh run; a changed task cannot receive the old result. No database transaction is held while the local suite runs.
...
Passing records simulated behavior, not live website health, content freshness, or external reachability. Learners still run `bash ~/scripts/site-check.sh "" maker-report.html` against the live site and inspect both pages in a browser. Existing stored completions remain historical and authoritative; the replacement neither regrades them nor treats them as evidence of passing this new suite.
